...
async function getSession(baseURL,token) {
    ...
    const response = await fetch(baseURL + '/api/sessions', options);
    return response;
}
 ...
 * @param baseURL MicroStrategy Library URL
 * @param token Authorization token
 * @param name Value the earch pattern is set to. Here, we use for search report objects whose name begins with user input.
